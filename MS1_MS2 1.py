...
def run_milestone1():
    ...
    sheets_to_update = ['Index', 'Entity TB', 'Validations', 'SoFP - Commentary', 'SoPL - Commentary', 'SoFP', 'SoPL',
                        'GN-18.1.1', 'CF Review form', 'GN-28', 'SOCIE', 'GN-17.3']
    ...
    for original_name, new_name in sheets_to_copy.items():
        ...
        if new_name in ["SoPL - Commentary_NEW", "SoFP - Commentary_NEW"]:
            ...
            # new_sheet.cell(row=1, column=18, value="row_no")
    ...
    if "GN-17.3_NEW" in final_wb.sheetnames:
        ...
        ws_gn173.cell(row=1, column=18, value="row_no")
        for col in range(2, 6):
    ...
    col_ref_sheet.cell(row=first_empty_row, column=4, value="Others")
       ...
       # Add new column "original_column_alphabet" at position 3
    col_ref_sheet = final_wb["Column_Reference"]
    col_ref_sheet.insert_cols(3)
    col_ref_sheet.cell(row=1, column=3, value="original_column_alphabet")
 
    # Fill the new column for all rows
    for row in range(2, col_ref_sheet.max_row + 1):
        col_number = col_ref_sheet.cell(
            row=row, column=2
        ).value  
        if isinstance(col_number, int):
            col_ref_sheet.cell(row=row, column=3, value=get_column_letter(col_number))
        else:
            col_ref_sheet.cell(
...
def run_milestone2():
    ...
    # Define keywords per sheet
    keyword_map = {
        "SoPL - Commentary": ["Revenues.", "Other operating expenses."],
        "SoFP - Commentary": [
    ...
    for sheet_name, keywords in keyword_map.items():
        if sheet_name in wb_in.sheetnames:
            ws_in = wb_in[sheet_name]
            ws_out_extract = wb_out.create_sheet(title=sheet_name)
            ...
            for row in ws_in.iter_rows(values_only=True):
                first_col_clean = clean_text(row[0]) if row and row[0] else ""

                # Match only if the cleaned cell exactly equals a cleaned keyword
                if any(first_col_clean == k for k in cleaned_keywords):
                    for col_index, value in enumerate(row, start=1):
                        ws_out_extract.cell(row=output_row, column=col_index, value=value)
                    output_row += 1


    #xtract Cover Page info (rows 7–11, columns B–C)
 
    if "Cover Page" in wb_in.sheetnames:
        ws_in = wb_in["Cover Page"]
        ws_out_cover = wb_out.create_sheet(title="Cover Page")

        headers = []
        values = []

        for row_num in range(7, 12):  # rows 7–11 inclusive
            header_val = ws_in.cell(row=row_num, column=2).value  # Column B
            value_val = ws_in.cell(row=row_num, column=3).value  # Column C
            headers.append(header_val)
            values.append(value_val)

        # Write headers and values as rows
        for col_index, header in enumerate(headers, start=1):
            ws_out_cover.cell(row=1, column=col_index, value=header)

        for col_index, val in enumerate(values, start=1):
            ws_out_cover.cell(row=2, column=col_index, value=val)

    # Save output
    wb_out.save("milestone2_output.xlsx")
    print(f"Milestone 2 output saved: milestone2_output.xlsx")
